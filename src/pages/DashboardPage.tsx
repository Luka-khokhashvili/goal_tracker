...
import { Badge, Button, Card, CardHeader, Modal, ProgressBar, Stat } from '@/components/ui';
...
function EmptyState({ onCreate }: { onCreate: ReturnType<typeof useGoals>['addGoal'] }) {
  const [open, setOpen] = useState(false);
  return (
    <Card className="mx-auto max-w-md text-center">
      <h2 className="text-lg font-semibold text-content">Start your savings goal</h2>
      <p className="mt-1 text-sm text-muted">
        Add the motorcycle you’re saving for and we’ll track every step to the finish line.
      </p>
...
function ActiveDashboard() {
  ...
  const { contributions, addContribution, updateContribution, deleteContribution } =
    useContributions();
  ...
  const [contribModal, setContribModal] = useState<{ open: boolean; editing?: Contribution }>({
    open: false,
  });
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        <CardHeader
          ...
          action={
            <Button variant="subtle" size="sm" onClick={() => setEditGoal(true)}>
              <Pencil size={15} /> Edit
            </Button>
          }
        />
        <div className="flex items-end justify-between">
          ...
          <p className="text-sm font-semibold text-content">{Math.round(percent)}%</p>
        </div>
        ...
        <Stat
          ...
          hint={
            <span className="flex items-center gap-1.5">
              ...
              <Badge tone={source === 'boost' ? 'warning' : source === 'override' ? 'brand' : 'neutral'}>
                {source === 'boost' ? 'reserve' : source === 'override' ? 'custom' : 'base'}
              </Badge>
            </span>
          }
        />
        ...
        <div className="flex flex-col gap-6">
          <Card>
            <CardHeader
              ...
              subtitle="Set in GEL"
              action={
                <Button variant="subtle" size="sm" onClick={() => setEditRule(true)}>
                  <Pencil size={15} /> Edit
                </Button>
              }
            />
            <dl className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd className="text-content">{formatFrom(rule.boostAmount, TARGET_CURRENCY)}</dd>
              </div>
            </dl>
          </Card>

          <Card>
            <CardHeader title="Exchange rates" subtitle="USD ⇄ GEL — used everywhere" />
      ...
      {/* Modals */}
      <Modal open={editGoal} onClose={() => setEditGoal(false)} title="Edit goal">
        <GoalForm
          goal={goal}
          onSubmit={(draft) => {
      ...
      <Modal open={editRule} onClose={() => setEditRule(false)} title="Edit monthly targets">
        <TargetRuleForm
          rule={rule}
          onSubmit={(draft) => {
            updateRule(draft);
      ...
      >
        <ContributionForm
          {...(contribModal.editing ? { contribution: contribModal.editing } : {})}
          onSubmit={(draft) => {
            if (contribModal.editing) updateContribution(contribModal.editing.id, draft);
